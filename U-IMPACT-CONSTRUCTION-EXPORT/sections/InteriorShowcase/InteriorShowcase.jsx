...
/**
 * InteriorShowcase — Architectural space and room exploration section.
 * Features alternating split and full-bleed layouts with fade reveals and parallax effects.
 * 
 * @param {Object} props
 * @param {string} props.headerLabel - Section category label
 * @param {string} props.headerTitle - Main headline (supports <br />)
 * @param {string} props.headerDescription - Supporting text
 * @param {Array} props.spaces - Array of space definition objects
 * @param {string} props.id - section ID (default: "spaces")
 */
...
export const DEFAULT_SPACES = [
  ...
  },
  {
    ...
    title: 'The Gymnasium',
    description:
      'More than a gym — it\'s where the community plays, celebrates, and grows together. From tournaments to community dinners, this versatile space adapts to every occasion.',
  ...
  },
  {
    ...
    title: 'Classrooms',
    description:
      'Purpose-built learning environments for studies, small groups, and workshops. Each room is equipped with modern technology and designed for meaningful dialogue and discovery.',
  ...
  },
  {
    ...
    description:
      'A world crafted just for kids — safe, joyful, and brimming with wonder. Age-appropriate spaces where youth discover learning through play, creativity, and mentorship.',
    accent: 'Where little hearts discover big dreams.',
    layout: 'full',
  ...
  },
  {
    ...
    description:
      'The operational heart that keeps everything running smoothly. Modern offices and meeting rooms where the leadership team plans, strategizes, and serves.',
  ...
  },
  {
    ...
    accent: 'Where connection happens every day.',
    layout: 'right',
      ...
      <div className={`container split ${space.layout === 'right' ? '' : 'split--reverse'}`}>
        ...
        <FadeSection direction={space.layout === 'right' ? 'left' : 'right'} delay={0}>
          <div className="interior-space__image-wrap">
            {space.image ? (
              <ParallaxImage src={space.image} alt={space.title} />
            ) : (
              <div
...
export default function InteriorShowcase({
  headerLabel = 'Explore the spaces',
  headerTitle = 'Every space tells\na story.',
  headerDescription = 'Designed with intention. Built with purpose. Discover the rooms that make this place home.',
  spaces = DEFAULT_SPACES,
  id = 'spaces',
}) {
  const titleLines = (headerTitle || '').split('\n');

  return (
    <section className="interior-showcase" id={id}>
      {/* Section header */}
